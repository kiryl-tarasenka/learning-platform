...
const navigation = [
  ...
  { href: "/progress", label: "Прогресс", description: "Учебный маршрут", icon: TrendingUp },
...
export function AppSidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 flex w-[264px] flex-col border-r border-slate-200 bg-[#f7f8fa] transition-transform md:static md:z-auto md:translate-x-0",
          collapsed && "md:w-[84px]",
      ...
      >
        <div className="flex h-16 items-center justify-between border-b border-slate-200 px-4">
          <Link className="flex items-center gap-3 overflow-hidden" href="/" onClick={() => setMobileOpen(false)}>
            <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-blue-600 text-white shadow-sm shadow-blue-200">
              <Code2 className="size-5" />
            </span>
            {!collapsed && (
              <span className="text-lg font-semibold tracking-tight text-slate-900">
                Front<span className="text-blue-600">Path</span>
              </span>
            )}
          </Link>
          <Button
        ...
        <div className="flex-1 px-4 py-8">
          {!collapsed && (
            <p className="mb-3 px-3 text-xs font-semibold tracking-wider text-slate-400 uppercase">Навигация</p>
          )}
              ...
              return (
                <Link
                  className={cn(
                    ...
                    active
                      ? "border-blue-600 bg-blue-100/70 text-blue-800"
          ...
          {!collapsed ? (
            <>
              <div className="mb-3 flex items-center justify-between">
                <span className="text-xs font-medium text-slate-500">Твой прогресс</span>
              ...
              <p className="mt-3 text-xs leading-5 text-slate-500">
                Продолжай в том же духе. Маленькие шаги складываются в навык.
              </p>
            </>
          ) : (
            <div className="mx-auto h-8 w-1.5 rounded-full bg-blue-500" />
          )}
        </div>
        <div className="border-t border-slate-200 p-5">
          <Link className="flex items-center gap-3 rounded-xl p-2 hover:bg-slate-50" href="/guide">
            <span className="grid size-9 place-items-center rounded-full bg-blue-100 text-sm font-semibold text-blue-700">
              К
            </span>
            {!collapsed && (
              <span className="min-w-0">
                <span className="block truncate text-sm font-medium text-slate-800">Курс frontend</span>
                <span className="flex items-center gap-1 text-xs text-slate-400">
                  <HelpCircle className="size-3" /> Нужна помощь?
                </span>
              </span>
            )}
          </Link>
